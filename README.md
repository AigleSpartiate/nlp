...
- The DiffSinger pipeline works as a "subprocess" in our project. It requires a specific Python version (3.8) and dependencies. You can install those and create the related project environment by running:
```bash
./external/DiffSinger/venvdiff/bin/pip install -r external/DiffSinger/requirements.txt
```
...
- Once this has been done you should be able to run the project. For instance, running:
```bash
python main.py --lyrics 小酒窝长睫毛是你最美的记号
```
in your terminal while in the root folder of the project generates an entire song based on the given lyrics. Please look inside main.py in order to see all available arguments you can use.
